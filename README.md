...
- 📊 **현재 한도 상태** — `status`가 지금 제한에 걸렸는지 화면에서 바로 확인하고, 리셋까지 남은 시간과 오늘/최근 7일 중단 횟수·총 중단 시간을 함께 표시
...
| `stop` | 감시 중지 |
| `status` | 감시 상태 + **현재 한도** + **중단 시점** + **tmux 세션 목록(AI CLI 실행 위치)** + 텔레그램 설정 + 최근 로그 |
...
| `usage` | 세션에 `/usage`를 입력해 사용량 확인 (**수동 실행 전용** — 감시는 호출하지 않음) |
...
`status`는 **지금** 한도에 걸려 있는지 화면에서 직접 확인하고, 누적 통계를 함께 보여줍니다.

```
== 사용 한도 ==
  ⛔ 제한 중:    리셋 07-27 17:25 (41분 남음) · %1
  오늘 2회 · 최근 7일 3회 중단 · 총 중단 3시간 37분 · 재개 미확인 1건
```
...
== 사용 한도 ==
  ✅ 제한 없음   (화면에 제한 메시지 없음)
  오늘 0회 · 최근 7일 3회 중단 · 총 중단 3시간 37분
```

- **API를 호출하지 않습니다.** AI CLI가 이미 화면에 출력한 제한 메시지와, autore가 직접 기록한 중단 이력만 사용합니다
- 리셋까지 남은 시간은 감시 루프와 같은 파서로 계산합니다 (`--buffer` 반영)
- 통계는 `~/.autore-breaks.log` 기준입니다. 재개 시각이 기록되지 않은 건(감시가 중간에 죽은 경우 등)은 `재개 미확인`으로 따로 셉니다
- 세션이 없으면 `- 확인 불가`로 표시됩니다 (통계는 그대로 표시)

## CLI에 직접 물어보기 (`autore usage`)

`status`의 한도 표시는 화면에 이미 떠 있는 정보만 읽습니다. **CLI가 아는 정확한 사용량**이 필요하면 이 명령으로 세션에 `/usage`를 직접 입력할 수 있습니다.

```bash
autore usage                 # 유휴 상태일 때만 /usage 입력 → 결과 출력 → Esc로 닫기
autore usage --keep-panel    # 패널을 닫지 않고 그대로 둠
autore usage --dry-run       # 무엇을 보낼지만 확인
```

**안전장치 — 이 명령은 세션에 키를 입력하므로 조건을 확인합니다:**

- **작업 중이면 거부합니다.** 화면에 중단 안내(`esc to interrupt`)가 있거나, 1.2초 간격으로 두 번 캡처한 화면이 다르면 작업 중으로 보고 아무것도 보내지 않습니다. 작업 중인 CLI에 키를 넣으면 다음 프롬프트로 제출될 수 있기 때문입니다
- **감시 루프와 `status`는 이 명령을 절대 호출하지 않습니다.** 사용자가 직접 실행할 때만 동작합니다
- 입력 전 `C-u`로 입력줄을 비우고, 조회 후 `Esc`로 패널을 닫습니다 (`--keep-panel`로 해제)
- CLI를 실제로 실행 중인 pane을 프로세스 트리로 찾아 그 pane에 보냅니다 (`--target`으로 고정 가능)

> ⚠️ `/usage`는 Claude Code의 명령이며, 이를 실행하면 CLI가 사용량 정보를 조회할 수 있습니다.
> autore 자체는 여전히 API를 호출하지 않지만, 이 명령은 **사용자를 대신해 CLI에 명령을 입력**한다는 점을 인지하고 사용하세요.
...
- `autore usage` 명령만 예외적으로 세션에 `/usage`를 입력합니다. 이는 **사용자가 직접 실행할 때만** 동작하며, 자동 감시는 호출하지 않습니다.
